...
export default function EMICalculator({
    ...
    const CustomSlider = ({ value, onChange, min, max, step = 1, label, formatValue }) => {
        // Ensure valid values
        ...
        const steps = Math.floor((safeMax - safeMin) / safeStep) + 1;
        ...
        const handleSliderChange = useCallback((clientX, rect) => {
            ...
            const rawValue = safeMin + percentage * (safeMax - safeMin);
            const steppedValue = Math.round(rawValue / safeStep) * safeStep;
            const clampedValue = Math.max(safeMin, Math.min(safeMax, steppedValue));
        ...
        return (
            <div className="relative pt-2">
                    ...
                    {label === 'Tenure' && (
                        <div className="absolute inset-0 flex justify-between items-center px-1 pointer-events-none">
                            {Array.from({ length: steps }, (_, i) => {
                                const stepValue = safeMin + (i * safeStep);
                                const isActive = safeValue >= stepValue;
                                return (
                                    <div
                                        key={i}
                                        className={`w-2 h-2 rounded-full transition-all duration-200 ${
                                            isActive ? 'bg-white border-2 border-[#D7242A]' : 'bg-gray-300'
                                        }`}
                                    />
                                );
                            })}
                ...
                <div className="flex justify-between mt-2 text-xs text-gray-500">
                    ...
                    <span className="font-semibold text-[#D7242A]">
                            ...
                            {showPriceSlider ? (
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-8 pr-4 py-3 border-2 border-gray-300 rounded-lg focus:border-[#D7242A] focus:outline-none text-lg font-semibold transition-colors"
                                    />
                                </div>
                            )}
